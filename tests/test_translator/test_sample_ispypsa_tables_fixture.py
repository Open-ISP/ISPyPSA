...
def test_sample_ispypsa_tables_fixture(sample_ispypsa_tables):
    ...
    assert (
        len(sample_ispypsa_tables["ecaa_generators"]) == 6
    )  # 2 coal + 4 REZ generators


def test_create_pypsa_friendly_inputs_with_fixture(
    sample_ispypsa_tables, sample_model_config
):
