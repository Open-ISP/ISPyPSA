...
def test_process_manual_custom_constraints_basic(csv_str_to_df):
    ...
    lhs, rhs, generators = _process_manual_custom_constraints(config, ispypsa_tables, links)
    ...
    lhs_sorted = lhs.sort_values(["constraint_name", "variable_name"]).reset_index(drop=True)
    expected_lhs_sorted = expected_lhs.sort_values(["constraint_name", "variable_name"]).reset_index(drop=True)
    
    rhs_sorted = rhs.sort_values("constraint_name").reset_index(drop=True)
    expected_rhs_sorted = expected_rhs.sort_values("constraint_name").reset_index(drop=True)
...
def test_process_manual_custom_constraints_missing_rhs(csv_str_to_df):
    ...
    with pytest.raises(ValueError, match="Incomplete manual custom constraints tables provided"):
...
def test_process_manual_custom_constraints_with_rez_expansion(csv_str_to_df):
    ...
    ispypsa_tables = {
        "custom_constraints_lhs": csv_str_to_df(custom_constraints_lhs_csv),
        "custom_constraints_rhs": csv_str_to_df(custom_constraints_rhs_csv),
        "rez_transmission_expansion_costs": csv_str_to_df(rez_transmission_expansion_costs_csv),
    }
    links = csv_str_to_df(links_csv)
    ...
    expected_generators_sorted = expected_generators.sort_values("name").reset_index(drop=True)
    
    # Compare all columns except capital_cost
    cols_to_compare = ["name", "isp_name", "bus", "p_nom", "p_nom_extendable", "build_year"]
    pd.testing.assert_frame_equal(
        generators_sorted[cols_to_compare],
        expected_generators_sorted[cols_to_compare]
    )
    ...
    # Compare LHS
    lhs_sorted = lhs.sort_values(["constraint_name", "coefficient", "variable_name"]).reset_index(drop=True)
    expected_lhs_sorted = expected_lhs.sort_values(["constraint_name", "coefficient", "variable_name"]).reset_index(drop=True)
    pd.testing.assert_frame_equal(lhs_sorted, expected_lhs_sorted)
...
def test_process_manual_custom_constraints_no_matching_links(csv_str_to_df):
    ...
    with pytest.raises(ValueError, match="The following link_flow terms reference links that don't exist: \\['NONEXISTENT'\\]"):
        _process_manual_custom_constraints(config, ispypsa_tables, links)
...
def test_process_manual_custom_constraints_multiple_no_matching_links(csv_str_to_df):
    ...
    with pytest.raises(ValueError, match="The following link_flow terms reference links that don't exist: \\['NONEXISTENT1', 'NONEXISTENT2'\\]"):
        _process_manual_custom_constraints(config, ispypsa_tables, links)
...
def test_process_manual_custom_constraints_empty_links_with_link_flow(csv_str_to_df):
    ...
    with pytest.raises(ValueError, match="The following link_flow terms reference links that don't exist: \\['PATH1'\\]"):
        _process_manual_custom_constraints(config, ispypsa_tables, links)
