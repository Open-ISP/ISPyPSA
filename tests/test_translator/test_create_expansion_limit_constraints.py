...
def test_create_expansion_limit_constraints_no_extendable_links(csv_str_to_df):
    ...
    # Call the function under test
    lhs, rhs = _create_expansion_limit_constraints(links, None, flow_paths, None)
...
def test_create_expansion_limit_constraints_all_none(csv_str_to_df):
    ...
    lhs, rhs = _create_expansion_limit_constraints(None, None, None, None)
...
def test_create_expansion_limit_constraints_missing_rhs_columns(csv_str_to_df):
    ...
    with pytest.raises(
        ValueError,
        match="RHS components missing required columns after processing: \\['constraint_name', 'rhs'\\]",
    ):
        _create_expansion_limit_constraints(links, None, flow_paths, None)
